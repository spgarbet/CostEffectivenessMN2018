...
## Handbook
In addition, as part of the [darthworkgroup.com](Decision Analysis in R for Technologies in Health \(DARTH\) working group), we have prepared a handbook titled "An Introduction to R for Decision Modelers". You can download the [https://github.com/DARTH-git/Handbook-R/blob/master/Handbook_in_R_markdown.pdf](handbook) to use as a refresher or a tutorial on the use of R prior to the workshop. You can download the data [https://raw.githubusercontent.com/DARTH-git/Handbook-R/master/Course_data.txt](here).
...
[https://drive.google.com/drive/folders/1xceAHNEg77EwFCjSW9NGePK6UslskQys?usp=sharing](Google Drive)
The solutions to the exercises will be uploaded daily by the end of each day to the following link: [https://drive.google.com/drive/folders/1yOqNdpVSs_n13c4_xHqQjCSk2VSALYl5?usp=sharing](Google Drive)
...
2010 ACA doesn't allow cost per QALY. [https://www.nytimes.com/2014/12/16/upshot/forbidden-topic-in-health-policy-debate-cost-effectiveness.html](New York Times article)
...
psa   <- t(apply(m.psa.input, 1, MM.SickSicker))
costs <- psa[,c(1, 3)]
qalys <- psa[,c(2, 4)]
...
MM.SickSicker <- function(params)
{
  with(as.list(params), {
    ...
    diag(cohort.tr)     <- 1 - rowSums(cohort.tr)
    markov(0:n.t, cohort.tr, c(1, 0, 0, 0))
  })
}

mle.SickSicker <- function(params)
{
  solution   <- MM.SickSicker(params)
  
  survival   <- 1-solution[2:(n.t+1),4]
  prevalance <- rowSums(solution[2:(n.t+1),c(2,3)]) / survival
  sick       <- solution[c(10, 20, 30), 2] / prevalance[c(10, 20, 30)]
...
lhs.grid <- gen_lhs()
cal   <- apply(lhs.grid, 1, mle.SickSicker)
hist(cal, freq=TRUE, main="Calibration Likelihoods")
```

```{r}
mle.lhs <- lhs.grid[which.max(cal),]
mle.lhs
```
...
solution <- MM.SickSicker(mle.lhs)
survival <- (1-solution[,4])[2:(n.t+1)]
lines(x=1:n.t, survival, col='red', lty=2, lwd=2)
```
...
plotrix::plotCI(x = 1:n.t, y = l.targets$Prev$value, 
                ...
                xlab = "Time", ylab = "Prevalance")
prevalance <- rowSums(solution[2:(n.t+1),c(2,3)]) / survival
lines(x=1:n.t, prevalance, col='red', lty=2, lwd=2)
```
...
plotrix::plotCI(x = l.targets$PropSick$Time, y = l.targets$PropSick$value, 
                ...
                xlab = "Time", ylab = "Proportion Sick")
sick <- solution[c(10, 20, 30), 2] / prevalance[c(10, 20, 30)]
lines(x=c(10, 20, 30), sick, col='red', lty=2, lwd=2)
```

```{r}
levels.vec <- round(quantile(cal, seq(0.1, 0.9, by=0.1)))

par(mfrow=c(1,3))

x <- seq(0.01, 0.5, by=0.01)        # p.S1S2
y <- seq(1,    4.5, by=0.07142857)  # hr.S1D
...
for(i in 1:50)
{
  for(j in 1:50)
  {
    ...
    ref$p.S1S2 <- x[i]
    ref$hr.S1D <- y[j]
    z[i,j] <- mle.SickSicker(ref)
  }
}

contour(x,y,z,
        levels=levels.vec,
        xlab='p.S1S2',ylab='hr.S1D')
points(mle.lhs$p.S1S2, mle.lhs$hr.S1D, pch = 2, col = "red", cex = 2)
legend("topright", c("LHS MLE"), col = c("red"), pch = c(2))

x <- seq(0.01, 0.5, by=0.01)    # p.S1S2
y <- seq(5,    15, by=10/49)  # hr.S2D
for(i in 1:50)
{
  for(j in 1:50)
  {
...
contour(x,y,z,
        levels=levels.vec,
        xlab='p.S1S2',ylab='hr.S2D')
points(mle.lhs$p.S1S2, mle.lhs$hr.S2D, pch = 2, col = "red", cex = 2)
legend("topright", c("LHS MLE"), col = c("red"), pch = c(2))

x <- seq(1,    4.5, by=0.07142857)  # hr.S1D
y <- seq(5,    15, by=10/49)        # hr.S2D
for(i in 1:50)
{
  for(j in 1:50)
...
contour(x,y,z,
        levels=levels.vec,
        xlab='hr.S1D',ylab='hr.S2D')
points(mle.lhs$hr.S1D, mle.lhs$hr.S2D, pch = 2, col = "red", cex = 2)
legend("topright", c("LHS MLE"), col = c("red"), pch = c(2))
```